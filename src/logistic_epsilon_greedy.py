...
from src.dummy import DummyModel
from src.fallback import FallbackModel
from src.model_switcher import ModelSwitcher
from src.slot_optimizer import SlotOptimizer
...
class LogisticEpsilonGreedy:
    ...
    def action(self, contexts: np.array=None, policy_state: Dict={}) -> pd.DataFrame:
        ...
        #if len(self.arms) <= len(self.slots):
        #    rankings.columns = self.slots[:len(self.arms)]
        #else:
            #rankings = rankings.iloc[:, :len(self.slots)]
    ...
    def update(self, action, reward, context=None, position=None) -> None:
        ...
        if self.optimize_slot:
            ...
            y = reward
            #w *= (y * pd.Series(position).map(self.pre_trained_position_bias)).replace({0: 1})
            ...
            #self._content_model.fit(feature, y)
